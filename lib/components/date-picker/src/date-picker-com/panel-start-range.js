...
const _sfc_main = /* @__PURE__ */ vue.defineComponent({
  ...
  setup(__props, { emit }) {
    ...
    const handleDatePick = (value, keepOpen = false) => {
      ...
      if (!nextDate)
        return;
      const { range, completed } = utils.getSequentialRangePick("start", selectingEndDate.value, nextDate, [minDate.value, maxDate.value]);
    ...
    const handleClear = () => {
      ...
      emit("pick", utils.getPartialRangePayload([void 0, remainingMaxDate]).pickRange);
    };
