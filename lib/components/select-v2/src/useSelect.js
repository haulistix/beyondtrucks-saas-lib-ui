...
const useSelect = (props, emit) => {
  ...
  const isOptionSelected = (option) => {
    ...
    if (!shared.isObject(optionValue)) {
  ...
  const popupHeight = vue.computed(() => {
    const totalHeight = filteredOptions.value.reduce((height, option, index) => {
      if (option.type === "Group") {
        ...
        return height + defaults.SELECT_V2_GROUP_TITLE_HEIGHT + (hasDivider ? defaults.SELECT_V2_GROUP_DIVIDER_SIZE : 0);
      }
      return height + props.itemHeight;
    }, 0);
    const selectionSectionHeight = props.multiple && !filteredOptions.value.some((option) => option.type === "Group") && hasVisibleSelectedOptions.value && hasVisibleUnselectedOptions.value ? defaults.SELECT_V2_GROUP_DIVIDER_SIZE + defaults.SELECT_V2_GROUP_TITLE_HEIGHT : 0;
  ...
  const checkBeforeChange = async (value, oldValue) => {
    ...
    if (!isPromiseOrBool) {
      error.throwError("ElSelectV2", "beforeChange must return type `Promise<boolean>` or `boolean`");
    }
