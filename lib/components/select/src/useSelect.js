...
const useSelect = (props, emit) => {
  ...
  vue.watch(() => props.modelValue, (val, oldVal) => {
    if (pendingAutoSelectValue !== noPendingAutoSelection && lodashUnified.isEqual(val, pendingAutoSelectValue)) {
  ...
  const setSelected = () => {
    ...
    const selectedValues = types.isUndefined(props.modelValue) ? [] : lodashUnified.castArray(props.modelValue);
    const result = [];
    optionsArray.value.forEach((option) => {
  ...
  const getLabelSlotItem = (item) => {
    ...
    if (cachedOption) {
      ...
      if (shared.isPlainObject(optionSource.rawOption)) {
        return optionSource.rawOption;
      }
      if (shared.isPlainObject(optionSource.value)) {
    ...
    return shared.isPlainObject(item.value) ? item.value : item;
  };
  ...
  const checkBeforeChange = async (value, oldValue) => {
    if (lodashUnified.isEqual(value, oldValue) || !props.beforeChange)
      return true;
    ...
    if (!isPromiseOrBool) {
      error.throwError("ElSelect", "beforeChange must return type `Promise<boolean>` or `boolean`");
    }
  ...
  const isMultipleOptionSelected = (option) => getValueIndex(lodashUnified.castArray(props.modelValue), option) > -1;
  ...
  const toggleSelectAll = async () => {
    ...
    let value = lodashUnified.castArray(props.modelValue).slice();
