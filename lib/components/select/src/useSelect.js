...
const useSelect = (props, emit) => {
  ...
  const tryAutoSelectSingleOption = () => {
    ...
    if (isEmptyValue(option.value))
      return;
    if (pendingAutoSelectValue !== noPendingAutoSelection && lodashUnified.isEqual(pendingAutoSelectValue, option.value)) {
      return;
    }
    pendingAutoSelectValue = option.value;
    emit(event$1.UPDATE_MODEL_EVENT, option.value);
