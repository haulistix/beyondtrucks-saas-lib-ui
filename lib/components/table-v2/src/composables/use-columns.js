...
function useColumns(props, columns, fixed, effectiveWidth, reservedVScrollbarWidth) {
  ...
  function onColumnSorted(e) {
    ...
    let order = constants.SortOrder.DESC;
    if (shared.isObject(sortState)) {
      order = constants.nextSortOrderMap[(_a = sortState[key]) != null ? _a : constants.SortOrder.DEFAULT];
    } else {
      order = sortBy.key === key ? constants.nextSortOrderMap[(_b = sortBy.order) != null ? _b : constants.SortOrder.DEFAULT] : constants.SortOrder.DESC;
    }
