...
const HeaderCellRenderer = (props, {
  ...
  if (sortState) {
    ...
    sorting = order === constants.SortOrder.ASC || order === constants.SortOrder.DESC;
    sortOrder = sorting ? order : constants.SortOrder.DESC;
  } else {
    sorting = column.key === sortBy.key && (sortBy.order === constants.SortOrder.ASC || sortBy.order === constants.SortOrder.DESC);
    sortOrder = sorting ? sortBy.order : constants.SortOrder.DESC;
  }
  const cellKls = [ns.e("header-cell"), diagonalHeader && ns.is("diagonal-header"), column.required && "required-column", column[_private.rowDeletePlaceholderMergedSign] && ns.is("row-delete-placeholder-merged"), utils.tryCall(headerClass, props, ""), column.align === constants.Alignment.CENTER && ns.is("align-center"), sortable && ns.is("sortable")];
  ...
  return vue.createVNode("div", vue.mergeProps(cellWrapperProps, {
    ...
    "sortOrder": sortOrder,
    "sorting": sorting
  }, null), column.resizable !== false && !(ghostTable && column[_private.rowDeletePlaceholderMergedSign]) && vue.createVNode("div", {
