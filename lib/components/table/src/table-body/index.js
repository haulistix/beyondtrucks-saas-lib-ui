...
var TableBody = vue.defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const { wrappedRowRender, tooltipContent, tooltipTrigger } = renderHelper["default"](props, emit);
  ...
  render() {
    ...
    const bodyRows = shouldRenderGhostRow ? (() => {
      ...
      return rows.concat(wrappedRowRender(ghostRow, rows.length));
    })() : rows;
    return vue.h("tbody", { tabIndex: -1 }, [bodyRows]);
  }
});
