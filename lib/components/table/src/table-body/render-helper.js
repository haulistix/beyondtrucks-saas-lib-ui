...
function useRender(props, emit) {
  ...
  const {
    ...
    tooltipContent,
    tooltipTrigger
  } = eventsHelper["default"](props, emit);
  const {
    getRowStyle,
    getRowClass,
    getCellStyle,
  ...
  const getKeyOfRow = (row, index) => {
    var _a, _b;
    if (row == null ? void 0 : row[_private.ghostRowSign]) {
      return (_a = row[_private.ghostRowKey]) != null ? _a : `${_private.ghostRowKey}_${index}`;
    }
  ...
  const rowRender = (row, $index, treeRowData, expanded = false) => {
    ...
    const rowClasses = [];
    const isGhostRow = Boolean(row == null ? void 0 : row[_private.ghostRowSign]);
    let display = true;
    if (treeRowData) {
