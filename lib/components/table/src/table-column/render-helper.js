...
var ghostRowAddButton = require('../ghost-row-add-button.js');
...
const hasGhostRowValue = (row) => {
  return Object.entries(row != null ? row : {}).some(([key, value]) => {
    if (key === _private.ghostRowSign || key === _private.ghostRowKey)
      return false;
...
const applyRequiredInputState = (vnodes, column, row) => {
  if (!column.required || !column.property)
    return vnodes;
  if ((row == null ? void 0 : row[_private.ghostRowSign]) && !hasGhostRowValue(row))
    return vnodes;
...
function useRender(props, slots, owner) {
  ...
  const setColumnWidth = (column) => {
    ...
    column.autoWidth = types.isUndefined(column.width) && !realWidth.value && !realMinWidth.value;
    if (column.autoWidth) {
      column.minWidth = columnWidth.getAutoColumnWidth(column.label);
  ...
  const setColumnRenders = (column) => {
    if (props.renderHeader) ; else if (column.type !== "selection") {
      column.renderHeader = (scope) => {
        ...
        return vue.renderSlot(slots, "header", scope, () => fallbackContent);
      };
    }
    ...
    if (column.type === "expand") {
      ...
      column.renderCell = (data) => {
        ...
        const shouldRenderAddButton = owner.value.props.ghostTable && owner.value.props.editTable && !!((_a = data.row) == null ? void 0 : _a[_private.ghostRowSign]) && (columnCount === 1 ? data.cellIndex === 0 : data.cellIndex === columnCount - 1);
        ...
        if (shouldRenderAddButton) {
          children = [
            vue.h(ghostRowAddButton["default"], {
              row: data.row
            })
          ];
        } else if (shouldRenderEditCell) {
          const vnodes = applyRequiredInputState(column.renderEditCell(data), column, data.row);
          const editVNodes = shared.isArray(vnodes) ? vnodes : [vnodes];
          children = editVNodes.some((v) => v.type !== vue.Comment) ? vnodes : originRenderCell(data);
        } else if (slots.default) {
          const vnodes = slots.default(data);
          const defaultVNodes = shared.isArray(vnodes) ? vnodes : [vnodes];
          children = defaultVNodes.some((v) => v.type !== vue.Comment) ? vnodes : originRenderCell(data);
        } else {
          children = originRenderCell(data);
        }
        const firstUserColumnIndex = columns.value.findIndex((item) => item.type === "default");
        const shouldCreatePlaceholder = hasTreeColumn.value && !((_b = data.row) == null ? void 0 : _b[_private.ghostRowSign]) && data.cellIndex === firstUserColumnIndex;
