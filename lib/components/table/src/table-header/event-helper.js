...
function useEvent(props, emit) {
  ...
  const isContentOverflowing = (element) => {
    ...
    const { top, left, right, bottom } = util.getPadding(element);
    return util.isGreaterThan(rangeWidth + left + right, elementWidth) || util.isGreaterThan(rangeHeight + top + bottom, elementHeight) || util.isGreaterThan(element.scrollWidth, elementWidth);
  };
  const handleCellMouseEnter = (event, row) => {
    ...
    if (summaryHeaderTitle) {
      ...
      } else if (((_d = util.removePopper) == null ? void 0 : _d.trigger) === cell) {
    ...
    if (isContentOverflowing(cellChild)) {
      util.createTablePopper({ effect: "light" }, (_f = (cell == null ? void 0 : cell.innerText) || (cell == null ? void 0 : cell.textContent)) != null ? _f : "", row, column, cell, table);
