...
const _sfc_main = /* @__PURE__ */ vue.defineComponent({
  ...
  setup(__props, { expose, emit }) {
    ...
    const emitInput = (input) => {
      if (!utils.valueEquals(props.modelValue, input)) {
        ...
        if (shared.isArray(input)) {
          formatted = input.map((item) => item ? utils.formatter(item, props.valueFormat, lang.value) : item);
        } else if (input) {
    ...
    const parsedValue = vue.computed(() => {
      ...
      if (shared.isArray(dayOrDays) && (dayOrDays.every((day) => !day) || !allowsPartialRange && dayOrDays.some((day) => !day))) {
        dayOrDays = [];
      }
      return dayOrDays;
    });
    ...
    const formatToString = (value) => {
      if (!value)
        return null;
      const res = shared.isArray(value) ? value.map((_) => _ ? _.format(props.format) : "") : value.format(props.format);
      return res;
    };
